...
import plugin.borealcore.cooking.configs.CookingConfigLoader;
import plugin.borealcore.cooking.configs.CookingMessage;
import plugin.borealcore.cooking.configs.CookingMessageLoader;
...
public class CookingModule extends Function implements BorealModule {
    ...
    public void onModuleEnable() throws Exception {
        ...
        context.getPlaceholderManager().registerExpansion(cookingPlaceholders);
        context.getPlaceholderManager().registerExpansion(competitionPlaceholders);
    ...
    public void onModuleDisable() throws Exception {
        ...
        if (this.cookingPlaceholders != null) context.getPlaceholderManager().unregisterExpansion(cookingPlaceholders);
        if (this.competitionPlaceholders != null) context.getPlaceholderManager().unregisterExpansion(competitionPlaceholders);
        activeFXTasks.clear();
        holograms.clear();
    ...
    public void handleMaterialAutocooking(String recipeId, Player player, Integer amount) {
        Ingredient recipe = null; //INGREDIENTS.get(recipeId); @TODO eee aah ahh
